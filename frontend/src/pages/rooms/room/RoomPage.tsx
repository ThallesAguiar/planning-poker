...
import { authHeaders, getRoomDefaults, loginAccount, logoutAccount, registerAccount } from "../../../api/auth";
import { useAppStore } from "../../../stores/app-store";
import { TableScreen } from "../../../components/room/TableScreen";
import { playerAvatars } from '../../../data/avatars';
import { entryCards } from '../../../data/room-decks';
...
export function App({ mode }: { mode: "home" | "room" }) {
  ...
  const { storageKey: roomStorageKey, clearJoinStorage } = useRoomSession(account?.id);
  ...
  }, [clearJoinStorage, clearState, mode, routeCode, setState, setAiStatus, setRoleRequests]);
  ...
  const submitAccount = async () => {
    ...
    try {
      ...
      setAccountSession(session.user, session.token, session.expiresAt);
