...
- [x] Frontend: tela `/rooms/:code/studio` estava espremida porque `RoomStudioPage` usava `dashboard-shell` sem o `<aside className="dashboard-nav">`, entao o conteudo caia na coluna de 230px do grid; substituido pelo componente `DashboardShell` (aside + content) em `frontend/src/pages/studio/room/index.tsx`; frontend `npm run build` e `npm run lint` aprovados.
- [x] Frontend: Studio da mesa (`/rooms/:code/studio`) alinhado ao layout de `/studio` — `RoomStudioForm` passou de tres `studio-subsection` empilhados para `studio-grid` de paineis `studio-panel` lado a lado (Provedor, Agente, Regras) e `RoomStudioPanel` deixou de envolver o form num painel extra; frontend `npm run build` e `npm run lint` aprovados.
- [x] Frontend: salas encerradas em `Minhas Salas` agora permitem `Entrar` (para continuar a discussao na mesma mesa) alem do link `Ver relatorio` quando existir; `renderAction` em `frontend/src/components/layout/DashboardPages.tsx` e estilo `.room-actions` (grid de dois botoes) em `frontend/src/index.css`; backend ja permitia join/present em sala encerrada. Frontend `npm run build` e `npm run lint` aprovados.
- [x] Salas nunca mais encerram automaticamente: ao concluir o backlog em `finalize()` (`api/src/realtime/round.service.ts`) a sala volta a `lobby`/`em_andamento` (antes virava `finalizada`/`encerrada`) e gerar relatorio (`api/src/reports/report.service.ts`) nao seta mais `status: 'encerrada'`; participantes ja aparecem em `Minhas Salas` via `GET /rooms/mine` (`room.service.ts:mine()`) com o `role` da mesa. API `npm test` (round+report), API `npm run build`, frontend `npm run build` e `npm run lint` aprovados.
- [x] Presenca estilo Zoom/Meet: ao desconectar (aba fechada, refresh, queda de rede) o participante sai da mesa imediatamente (`disconnectGraceMs` padrao agora `0` em `api/src/room.gateway.ts`, mantendo `ROOM_DISCONNECT_GRACE_MS` como override); refresh reconecta e reentra com o token salvo; API `npm test` completo (22 arquivos, 166 testes) aprovado.
- [x] Presenca estilo Zoom/Meet (parte 2): `pruneDisconnected()` em `api/src/room.gateway.ts` remove do roster vivo participantes desconectados (sem socket ativo) ao restaurar snapshot Redis ou carregar do banco, incluindo a IA participante inativa (sala 0339C8 tinha "Jogador" e "Qodex" presos com `connected:false`); linhas de `RoomParticipant` preservadas para rejoin/"Minhas Salas"; teste de regressao adicionado em `api/src/room.gateway.spec.ts`; API `npm test` completo (22 arquivos, 167 testes) e `npm run build` aprovados. Container `planning-pocker` na 3333 pendente de rebuild para aplicar em runtime.

- [x] Studio da mesa: conta pode assumir, de forma idempotente, mesa que ela própria criou como visitante usando somente JWT visitante local da mesma sala. `POST /rooms/:id/claim-owner` rejeita token ausente, inválido, de outra sala ou participante que não é dono. Tela ganhou cabeçalho, aviso contextual, badge da mesa e retorno alinhados ao Studio da conta. API e frontend recompilados, containers rebuildados e Playwright `studio-route.spec.ts` aprovado (3 testes, incluindo visitante → conta).

- [x] Autenticação: refresh token opaco e rotativo em cookie HTTP-only, hash persistido em `RefreshSession`, revogação no logout e renovação automática do Bearer cinco minutos antes do vencimento. Migration `20260919103000_refresh_sessions` aplicada; API build/lint e 13 testes de auth aprovados; smoke HTTP confirmou rotação do access token e preservação do usuário; frontend build/lint e Playwright completo 24/24 aprovados.
- [x] Salas: dono autenticado pode excluir sua sala por `DELETE /rooms/:id`; exclusão remove dados vinculados por cascata e encerra sockets/snapshots vivos sem derrubar API, inclusive corrida de desconexão e eleição de dono. `Minhas Salas` exibe `×` no canto superior direito somente para dono, com modal estilizado de perda permanente e `title` nas ações. CORS preflight de `http://localhost:5173` para Bearer confirmado (`204`, `Access-Control-Allow-Headers: authorization`); API build/lint, 30 testes focados (`room`, `presence`, `gateway`), frontend build/lint e Playwright `auth-rooms.spec.ts` (3 testes) aprovados; API saudável após teste de exclusão.
- [x] IA/Studio: idioma de resposta por agente (`pt-BR` padrão ou `en`) persistido em `AiAgent` e `RoomAiAgent`; mesa herda idioma da conta ao herdar agente e sobrescreve com agente próprio. Migration `20260919113000_ai_response_language` aplicada; prompts de voto, discussão e resumo exigem idioma configurado. API build/lint, 44 testes de IA, frontend build/lint, Playwright Studio 4/4 e smoke REST de persistência aprovados; containers rebuildados.
